...
const findUnspentTxOut = (transactionId: string, index: number, aUnspentTxOuts: UnspentTxOut[]): UnspentTxOut => {
    const foundTxOut = aUnspentTxOuts.find((uTxO) => uTxO.txOutId === transactionId && uTxO.txOutIndex === index);
    if (!foundTxOut) {
        throw new Error(`UnspentTxOut not found for transactionId: ${transactionId} and index: ${index}`);
    }
    return foundTxOut;
};
...
const signTxIn = (transaction: Transaction, txInIndex: number,
    ...
    const signature: string = toHexString(new Uint8Array(key.sign(dataToSign).toDER()));
